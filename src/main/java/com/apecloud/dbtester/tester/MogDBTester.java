...
public class MogDBTester implements DatabaseTester {
    ...
    // 接收 DBConfig 的构造函数
    public MogDBTester(DBConfig dbConfig) {
    ...
    public DatabaseConnection connect() throws IOException {
        ...
        try {
            ...
            throw new RuntimeException("Mogdb JDBC Driver not found, please try again..", e);
        }

        String url = String.format("jdbc:mogdb://%s:%d/%s?useSSL=false&serverTimezone=UTC",
                dbConfig.getHost(),
                dbConfig.getPort(),
                dbConfig.getDatabase());

        String url2 = String.format("jdbc:mogdb://%s:%d/%s?useSSL=false&serverTimezone=UTC",
                dbConfig.getHost(),
                dbConfig.getPort(),
                databaseConnection);
        ...
        try {
            return new MogdbConnection(DriverManager.getConnection(url, dbConfig.getUser(), dbConfig.getPassword()));
        } catch (SQLException e) {
            System.err.println("Failed to connect to Mogdb database: " + e );
            System.err.println("Trying with database mogdb.");
            try {
                return new MogdbConnection(DriverManager.getConnection(url2, dbConfig.getUser(), dbConfig.getPassword()));
            } catch (SQLException e2) {
                throw new IOException("Failed to connect to Mogdb database: ", e2);
            }
        }
    }
    ...
    public QueryResult execute(DatabaseConnection connection, String query) throws IOException {
        MogdbConnection postgresConnection = (MogdbConnection) connection;
        try {
    ...
    private void executeBenchmark(DatabaseConnection connection, String query) throws IOException {
        MogdbConnection conn = (MogdbConnection) connection;
    ...
    private static class MogdbConnection implements DatabaseConnection {
        private final Connection connection;

        MogdbConnection(Connection connection) {
        ...
        public void close() throws IOException {
            try {
                ...
                throw new IOException("Failed to close Mogdb connection", e);
            }
        }
    }

    public static class MogdbQueryResult implements QueryResult {
        private final ResultSet resultSet;
        private final int updateCount;

        MogdbQueryResult(ResultSet resultSet, int updateCount) {
