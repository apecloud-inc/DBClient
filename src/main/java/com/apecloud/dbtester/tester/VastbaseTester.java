...
public class VastbaseTester implements DatabaseTester {
    ...
    // 默认构造函数
    public VastbaseTester() {
        this.dbConfig = null;
    }

    // 接收 DBConfig 的构造函数
    public VastbaseTester(DBConfig dbConfig) {
    ...
    public DatabaseConnection connect() throws IOException {
        ...
        try {
            ...
            throw new RuntimeException("Vastbase JDBC Driver not found, please try again..", e);
        }
        ...
        String url2 = String.format("jdbc:opengauss://%s:%d/%s?useSSL=false",
                dbConfig.getHost(),
                dbConfig.getPort(),
                databaseConnection);
        ...
        try {
            return new VastbaseConnection(DriverManager.getConnection(url, dbConfig.getUser(), dbConfig.getPassword()));
        } catch (SQLException e) {
            System.err.println("Failed to connect to Vastbase database: " + e);
            System.err.println("Trying with database postgres.");
            try {
                return new VastbaseConnection(DriverManager.getConnection(url2, dbConfig.getUser(), dbConfig.getPassword()));
            } catch (SQLException e2) {
                throw new IOException("Failed to connect to Vastbase database: ", e2);
            }
        }
    }
    ...
    public QueryResult execute(DatabaseConnection connection, String query) throws IOException {
        VastbaseConnection vastbaseConnection = (VastbaseConnection) connection;
        try {
            Statement statement = vastbaseConnection.connection.createStatement();
            boolean isResultSet = statement.execute(query);
            return new VastbaseQueryResult(statement.getResultSet(), statement.getUpdateCount());
        } catch (SQLException e) {
    ...
    private void executeBenchmark(DatabaseConnection connection, String query) throws IOException {
        VastbaseConnection conn = (VastbaseConnection) connection;
    ...
    private static class VastbaseConnection implements DatabaseConnection {
        private final Connection connection;

        VastbaseConnection(Connection connection) {
        ...
        public void close() throws IOException {
            try {
                ...
                throw new IOException("Failed to close Vastbase connection", e);
            }
        }
    }

    public static class VastbaseQueryResult implements QueryResult {
        private final ResultSet resultSet;
        private final int updateCount;

        VastbaseQueryResult(ResultSet resultSet, int updateCount) {
...
//            .table("test_table")
            .build();
        VastbaseTester tester = new VastbaseTester(dbConfig);
